...
工程上，我们可以在代码中显式构建以下标量函数：$$\mathcal{L}_{\text{surr}}(\theta) = \frac{1}{M} \sum_{m=1}^{M} \left( E_l(X_m) - \bar{E}_l \right) \cdot \log |\psi_\theta(X_m)|^2$$
...
理论很美好，但这个东西在工程上问题不小。如果模型有 $N$ 个参数，费舍尔信息矩阵 $F$ 的大小就是 $N \times N$。计算 $F$ 的逆矩阵 $F^{-1}$ 的时间复杂度达 $O(N^3)$，内存消耗 $O(N^2)$，这在工程上是无法直接承受的。


## 3. 结果分析

这部分就不写了，神经网络这玩意懂得都懂，论文中肯定只会拿具有说服力的结果，无法给出泛用性的结论。自己对着论文复述一遍也没有意义。
